#!/usr/bin/env bash
# Generate release artifacts and checksums for gittool.
# Usage: scripts/release_checksums.sh --version v1.0.4
set -euo pipefail
...
if [[ -z "$VERSION" ]]; then
  echo "Missing --version argument (e.g. --version v1.0.4)" >&2
  exit 1
fi
